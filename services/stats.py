    ...
    def measures_of_dispersion(self):
        for col in self.df.columns:
            if pd.api.types.is_numeric_dtype(self.df[col]):
                ...
                self.feature_wise_measures[col]["Q1"]=np.percentile(self.df[col], 0.25)
                self.feature_wise_measures[col]["Q2"]=np.percentile(self.df[col], 0.5)
                self.feature_wise_measures[col]["Q3"]=np.percentile(self.df[col], 0.75)
